...
#region 脚本工具生成的代码
        ...
        public async void MatchBtnEvent() { // 异步方法
            bool isMatchSucceed = await UIComponent.GetUiView<MatchingRoomPanelComponent>().StartMatch(CardFiveStarRoom.Ins._RoomId); // 先等待异步结果：先前要求的匹配匹配成功了吗？
            if (isMatchSucceed) 
                CutMatchingUI();
        }
        // 切换房卡刚进入房间准备中的UI
