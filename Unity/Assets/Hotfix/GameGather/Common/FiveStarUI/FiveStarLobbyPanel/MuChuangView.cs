...
namespace ETHotfix {
    ...
    public class LanternAnim { // 给灯笼一点儿随飘动的动画效果
        ...
        public async void PlayAnim() { // 播放动画
            if (isAnimIn) { // 已经播放中
                return;
            }
            isAnimIn = true;
            while (gameObject.activeInHierarchy) { // 这哪里是什么协程，就是一个异步等待2 秒钟呀
                ...
                await ETModel.Game.Scene.GetComponent<TimerComponent>().WaitAsync(2000); // 这个动画效果是；每隔2 秒钟，就重复播放一次呀，小风刮呀刮。。
            }
            isAnimIn = false;
        }
    }
...
#region 脚本工具生成的代码
        ...
        public void CreateRoomBtnEvent() { // 创建房间：
