...
#region 脚本工具生成的代码
        ...
        private async void InitRoomList() { // 启动房间链表：那么服务器可以承载同步多少个房间呢？同时在线承压的话
            if (_MatchRoomConfigs==null) {
                await RequestMatchRoomConfigs(); // 如果匹配房间配置没有，就会请求服务器
            }
        ...
        public async ETTask<MatchRoomConfig> GetMatchRoomConfig(int roomId) {
            ...
            for (int i = 0; i < _MatchRoomConfigs.Count; i++) {
                if (_MatchRoomConfigs[i].MatchRoomId == roomId) { // 返回的是，房间链表中，这个匹配的，拿到的特定房间的配置
