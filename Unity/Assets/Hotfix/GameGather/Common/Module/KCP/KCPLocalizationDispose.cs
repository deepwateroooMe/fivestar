...
namespace ETHotfix {
    ...
    public class KCPLocalizationDispose : Component {
        public void Awake() {
            ...
            KCPStateManage.Ins.pConnectSuccessCall += ConnectSuccessEvent; // 这里，就是感兴趣的订阅者，订阅了回调事件
        ...
        private void ConnectSuccessEvent(G2C_GateLogin g2CGateLogin) {
            UIComponent.GetUiView<LoadingIconPanelComponent>().Hide(); // 隐藏了负责显示加载的UI面板LoadingIconPanel
			User user = g2CGateLogin.User;
            Game.Scene.GetComponent<UserComponent>().SetSelfUser(user); ;
            Game.Scene.GetComponent<ToyGameComponent>().StartGame(ToyGameId.Lobby); // 通过StartGame方法进入大厅的通道，同时开始心跳
