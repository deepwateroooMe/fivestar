...
namespace ETHotfix {
    ...
    public static class KCPNetWorkState {
        ...
        public const string Disconnectl = "Disconnectl"; // 为什么会写得这么怪异呢？
    ...
    public class KCPStateManage : Component {
        ...
        public async void ConnectLost() {
            Log.Debug("连接断开");
            pKCPNetWorkState = KCPNetWorkState.Disconnectl;
            await ETModel.Game.Scene.GetComponent<TimerComponent>().WaitAsync(1000);// 连接断开后等待1秒 才发起重连 因为底层需要对Session进行一些处理【可以去看下底层在干嘛】
